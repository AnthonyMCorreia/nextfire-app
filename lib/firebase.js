...
export function getUserPictures(uid) {
	const userStorage = ref(storage, `uploads/${uid}`)
	let list = []

	listAll(userStorage).then(({ items }) => {
		list = items.map((item) => {
			const resolved = getDownloadURL(item).then((item) => item)
			return resolved
		})

		console.log("proimse.all", Promise.all(list))
	})
